...
import * as scroll from "./lister.js";
// 
...
export class page extends outlook.baby<crud_result> {
    ...
    constructor(
        ...
        //administered.
        const Theme = new scroll.theme(this,subject, "#content")
    ...
    private get_admin_parameters(button: HTMLButtonElement): admin_parameters {
        ...
        const Theme = <scroll.theme>this.panels.get("theme")!;
        //
    ...
    static mark_as_edited(evt: Event|HTMLElement): void {
        ...
        //
        //get the td' io
        const io = scroll.lister.ios.get(String(pos))!;
        //
    ...
    get theme() {
        return <scroll.theme>this.panels.get("theme")!;
    }
    //
    ...
    private * collect_inputs() {
        ...
        for(let td of tds) {
            ...
            const Io = scroll.theme.ios.get(String([this.theme.key,rowindex,cellIndex]));
            // 
            //
            if (Io===undefined) {
    ...
    async delete(): Promise<void> {
        ...
        const records = await server.exec("mysql", [dbname], "query", [sql]);
        //
