...
//format.
export class lister extends panel {
    layout;
    // 
    ...
    layout) {
        ...
        this.stylesheet = create_element(this.document.head, "style", {}).sheet;
    }
    //
    ...
    async paint() {
        ...
        //is tabular
        if (this.layout.type === "tabular")
            this.header.forEach(tn => tn.paint());
        // 
    ...
    async get_header() {
        ...
        const entries = cnames.map((cname, dposition) => {
            ...
            Tin.anchor.onclick = (evt) => this.select_column(evt);
            //
        ...
        entries.sort((a, b) => a[1].dposition - b[1].dposition);
        //
...
// 
export class scroller extends lister {
    Udf_meta;
    //
 ...
 */
    ...
    //
    selection;
    constructor(base, css, layout, Udf_meta) {
        super(css, base, layout);
        this.Udf_meta = Udf_meta;
    }
    //
    ...
    async query(offset, limit) {
        ...
        return await server.exec("mysql", 
        //
...
class barrel {
    ...
    constructor(Lister) {
        ...
        this.id = this.anchor.rowIndex;
    }
    // 
    // 
    paint() { this.tins.forEach(Tin => Tin.paint()); }
    // 
    get_parent() { return this.Lister; }
}
// // 
...
export class theme extends scroller {
    ...
    get sql() { return { stmt: this.sql_, dbname: this.subject[1] }; }
    //
    //Get the column names of this panel
    async get_col_names() { return this.col_names; }
    //
    ...
    async get_Ifuel() {
        ...
        let pk;
        pk = (this.selection !== undefined ? this.selection.pk : 0);
        //
        //
