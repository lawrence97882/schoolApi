...
import { column_meta,sch_library } from "./library.js";
// 
// 
//
...
// 
//These are the user defined metadatass
interface udf_meta{
  cname: string,
  uposition?: number,
  hidden?: boolean,
...
type offset = number;
// 
//The direction of scroll as implemented now is either the top or bottom.
...
//format.
export abstract class lister extends panel{
    // 
  ...
  abstract get_io(header_tin:tin):io.io;
  //
  ...
  constructor(
    ...
    public layout: layout
        ...
        this.stylesheet= <CSSStyleSheet>create_element(this.document.head,"style",{}).sheet;
    }
  //
  ...
  async paint(): Promise<void>{
        ...
        //is tabular
        if (this.layout.type=== "tabular")this.header.forEach(tn=>tn.paint());
        // 
        //Get Ifuel i.e.,the data to paint to the body 
  ...
  async get_header(): Promise<Map<library.cname, tin>>{
    ...
    //Map the column names to their entries, i.e.,column name/tin pairs
     const entries:Array<[library.cname,tin]>= cnames.map((cname,dposition)=>{
        ...
        Tin.io = new io.readonly(Tin.anchor);
        //
  ...
  //
    ...
    private select_column(evt: Event | HTMLTableHeaderCellElement){
        //
        //0. Get the target th.
        const th = evt instanceof Element
  ...
  paint_body(Ifuel:library.Ifuel):void { 
        ...
        for(const Ibarrel of Ifuel) {
            ...
            for (const htin of this.header!){
                ...
                const Io = this.get_io(tn);
                Tin.io=Io;
                //
                //Save this io 
                const name = `${this.key}${body_barrel.id}${Tin.id}`;
                lister.ios.set(name,Io);
                //
                //Get the value to be associated with this tin
                const value= Ibarrel[tn.dposition!];
                //
                //Set the tins value
                Tin.value = value; 
                // 
...
// 
export abstract class scroller extends lister{
    //
    ...
    //on the outcome type.
    private async execute_outcome(outcome: outcome, request: number) {
        //
        switch (outcome.type) {
            //
...
//           case "region":
//            return new io.select(anchor, ["page", "verticles", "horizantals", "intersects"]);
//           case "length": 
//             return new io.input("number", anchor);
//           default:
//             return new io.readonly(anchor);
...
class tin{
  ...
  // 
  //An id that is used to identify the position of this tin 
  public id:number;
  //
  // 
  ...
  constructor(protected Barrel:barrel) {
    ...
    this.id=(<HTMLTableCellElement>this.anchor).cellIndex;
  }
  paint() {
...
class barrel{
  ...
  // 
  //A number that uniquely identifies this barrel.
  public id:number;
  // 
  public anchor: HTMLElement;
  constructor(protected Lister:lister) {
    this.anchor = create_element(Lister.table!.tBodies[0], "tr", {});
    this.id=(<HTMLTableRowElement>this.anchor).rowIndex;
  }
  // 
  // 
...
// //
//
export class theme extends scroller{
    // 
    ...
    //theme panel.
      ...
      super(base, css,layout===undefined? {type:"tabular"}:layout)
    }
    //
    ...
    // 
    //Set the sql of a theme 
    get sql(): sql{return {stmt:this.sql_!, dbname:this.subject[1]}}
    //
    ...
    async get_Ifuel(): Promise<library.Ifuel>{
        ...
        let pk: library.pk;
        pk= <number>(this.selection !== undefined ? this.selection.pk : 0);
        //
        //
        return await this.query(pk, this.config.limit);
    }
    //
    //Returns the io of the given header tin
    get_io(header_tin:tin): io.io {
        ...
        const col = <schema.column>this.dbase!.entities[ename].columns[col_name];
        //
    ...
    //
    //The 
    get_udf_meta():{ [cname: string]: udf_meta }{return {}}
}
