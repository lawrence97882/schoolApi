...
import { sch_library } from './library';
import * as lister from "./lister.js"
//
...
export abstract class app extends outlook.view{
    ...
    async initialize():Promise<void>{
        ...
        this.panels.set("services", <lister.panel> new services(<outlook.view>this));
        //
        //Set the theme panel
        this.panels.set("theme", new lister.theme(this,this.subject, "#content"))
        //
    ...
    async set_dbase(){
        ...
        //const idbase = await this.exec("database", [this.dbname],"export_structure",[]);
        const idbase= await server.exec("mysql", [this.dbname],"export_structure",[])
        //
  ...
  //storage hence no loging in is required. 
    async login(User ?: outlook.user){
        ...
        const ids =<Array<{id:string}>> await server.exec("mysql",[this.dbname],"get_sql_data",[sql])
        // 
    ...
    async activate_products(): Promise<void>{
        ...
        const subscribed=<Array<{role_id:string,product_id:string}>>await server.exec("app", [this.id], "subscribed_products", [this.user!.email!]);
        // 
        //Add the subscribed
...
export class services extends lister.panel{
  //
